...
ShaderProgram::ShaderProgram(GLWidget277 *context)
    : vertShader(), fragShader(), prog(),
      ...
      //-HB
      unifTimeCol(-1), unifTimePos(-1), unifRenderMode(-1), unifPosMode(-1)
{}
...
void ShaderProgram::create(const char *vertfile, const char *fragfile)
{
    ...
    char* vertSource = new char[qVertSource.size()];
    strcpy(vertSource, qVertSource.toStdString().c_str());
    char* fragSource = new char[qFragSource.size()];
    ...
    unifTimeCol = context->glGetUniformLocation(prog, "u_TimeCol");
    unifTimePos = context->glGetAttribLocation(prog, "u_TimePos");
    unifPosMode = context->glGetAttribLocation(prog, "u_PosMode");
}
...
//HB: from here
void ShaderProgram::setRenderMode(int mode) {
    useMe();

    if (mode != -1) {
        context->glUniform1i(unifRenderMode, mode);
    }
}

void ShaderProgram::setColTime(int time) {
    useMe();

    if (time != -1) {
        context->glUniform1i(unifTimeCol, time);
    }
}


void ShaderProgram::setPosTime(int time) {
    useMe();

    if (time != -1) {
        context->glUniform1i(unifTimePos, time);
    }
}

void ShaderProgram::setPosMode(int mode) {
    useMe();

    if (mode != -1) {
        context->glUniform1i(unifPosMode, mode);
    }
}
//HB: to here.

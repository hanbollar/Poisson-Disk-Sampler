#include "ignoreGeoLight.h"
#include <iostream>
#include <math.h>
#include <warpfunctions.h>

float IgnoreGeoLight::Area() const
{
    //DELETEME
    return 4.f * Pi * transform.getScale().x * transform.getScale().x; // We're assuming uniform scale
}

void IgnoreGeoLight::ComputeTBN(const Point3f& P, Normal3f* nor, Vector3f* tan, Vector3f* bit) const
{
...
bool IgnoreGeoLight::Intersect(const Ray &ray, Intersection *isect) const
{
    //Transform the ray
...
Point2f IgnoreGeoLight::GetUVCoordinates(const Point3f &point) const
{
...
Intersection IgnoreGeoLight::Sample(const Intersection &ref, const Point2f &xi, float *pdf) const
{
...
Intersection IgnoreGeoLight::Sample(const Point2f &xi, Float *pdf) const
{
...
Bounds3f IgnoreGeoLight::WorldBound() const {
    ...
    Point3f min = Point3f(-1.0f, -1.0f, -1.0f);
    Point3f max = Point3f(1.0f, 1.0f, 1.0f);

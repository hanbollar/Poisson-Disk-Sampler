...
Ray Camera::Raycast(Point2f &pt) const
{
...
Ray Camera::RaycastNDC(float ndc_x, float ndc_y) const
{
    ...
    if (lensRadius > 0) {
        ...
        Point3f pLens = lensRadius * WarpFunctions::squareToDiskConcentric(samp->Get2D());

        //Compute point on plane of focus
        Vector3f dir = result.direction;
        float ft = focalDistance / dir.z;
        Point3f pFocus = dir * (ft);

        //Update ray for effect of lens
        result.origin = eye + Point3f(pLens.x, pLens.y, 0.0f);
        result.direction = glm::normalize(pFocus - (result.origin - eye));
    }

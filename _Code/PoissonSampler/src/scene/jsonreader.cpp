...
//        shape = std::make_shared<Mesh>();
    ...
    else if (QString::compare(type, QString("Ignore")) == 0) {
        shape = std::make_shared<IgnoreGeoLight>();
    }
    else
    {
        std::cout << "Could not parse the geometry!" << std::endl;
    ...
    else if(QString::compare(lgtType, QString("PointLight")) == 0)
    {
        Color3f lightColor = ToVec3(geometry["lightColor"].toArray());
        Float intensity = static_cast< float >(geometry["intensity"].toDouble());
        shape->transform = Transform(shape->transform.position(), glm::vec3(0.0f), glm::vec3(0.005f));
    ...
    else if(QString::compare(lgtType, QString("SpotLight")) == 0)
    {
        ...
        float TotW = static_cast< float >(geometry["totalWidth"].toDouble());
        float FallOffSt = static_cast< float >(geometry["fallOffStart"].toDouble());
        lightType = std::make_shared<SpotLight>(shape->transform, lightColor * intensity, TotW, FallOffSt);
    }
    else
    {
        std::cout << "Could not parse the geometry!" << std::endl;
